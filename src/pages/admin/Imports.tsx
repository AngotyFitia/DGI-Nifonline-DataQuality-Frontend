...
import { useGeoImport } from "../../hooks/useImport";
...
export default function Imports() {
  ...
  const { uploadAll, loading } = useGeoImport();

  // état pour les rapports JSON
  const [reports, setReports] = useState<Record<FileKey, ImportReport | null>>({
    provinces: null,
    regions: null,
    districts: null,
    communes: null,
    ...
    return (
      <div className="flex flex-col items-center gap-3 py-6">
        ...
        <p className="text-sm text-[var(--text-secondary)]">
          Importer fichier {label.toUpperCase()}
        </p>
        <Button
          variant={file ? "success" : "secondary"}
        ...
        <input
          ...
          accept=".csv,.xlsx"
          className="hidden"
          onChange={(e) => handleFileChangeLocal(e, key)}
        />
      </div>
    );
  };
  ...
  const handleImport = async () => {
    try {
      ...
      // si tu veux vider après un import réussi
      setFiles({
        provinces: null,
        regions: null,
        districts: null,
  ...
  {tabs.map((t) => {
    ...
    return <Alert key={t.key} type={type} message={msg} />;
      ...
      <DashboardCard title="Informations système">
        <Alert
          type="info"
          message="Chaque entité peut être importée via un fichier séparé"
        />
        <Alert
          type="warning"
          message="Les données sont liées hiérarchiquement (Province → Région → District → Commune)"
        />
      </DashboardCard>
    </div>
  );
