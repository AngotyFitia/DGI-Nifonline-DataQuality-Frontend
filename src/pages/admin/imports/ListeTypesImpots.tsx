...
import { useTypesImpots } from "../../../hooks/useImport";
import { importTypesImpots } from "../../../services/typesImpotsService";
import { CheckCircle, XCircle, Upload } from "lucide-react";
import type { ImportReport } from "../../../types/import";

export default function ListeFormesJuridiques() {
    const [code, setAbreviation] = useState("tous");
    ...
    const { data, isLoading } = useTypesImpots(token, code, intitule, etat, page, size);
    ...
    const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const token = localStorage.getItem("jwt");
        try {
        const report: ImportReport = await importTypesImpots(token, file);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-semibold">Liste des types d'impôts </h2>
        <div>
        ...
        <div className="mt-4">
          <Alert
            ...
            message={
              <>
                <p>
                  {`Import types d'impôt : ${importReport.success}/${importReport.total} réussis, ${importReport.error} erreurs.`}
                </p>
      ...
      <DashboardCard title="">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <Input placeholder="Code..." value={code === "tous" ? "" : code}onChange={(e) => { setAbreviation(e.target.value || "tous"); setPage(0);}}/>
