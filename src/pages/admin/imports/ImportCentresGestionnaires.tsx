...
import { useUploadCentres } from "../../../hooks/useImport";
...
export default function ImportCentresGestionnaires() {
  ...
  const [files, setFiles] = useState<FilesState>({coordonnees: null, centres: null});
  const token = localStorage.getItem("jwt") || undefined;
  const uploadCenresMutation = useUploadCentres(token);
  const { mutate: uploadAll, status } = uploadCenresMutation
